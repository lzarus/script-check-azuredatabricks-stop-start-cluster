...
function accessToken {
    ...
    StopDatabricksCluster $ClusterConfigurations $accessToken
}
function StopDatabricksCluster ($ClusterConfigurations, $accessToken) {
  foreach ($configuration in $ClusterConfigurations) {
    ...
    if (($cluster.state -eq "TERMINATING") -or ($cluster.state -eq "TERMINATED")) {
      Write-Output "Cluster $DatabricksClusterName is already stopped"
    }
    else {
      ...
      Write-Output "Stopped cluster $DatabricksClusterName in Databricks workspace $DatabricksWorkspaceName"
    }
  }
}
authAzure